...
namespace DakHome
{
    public partial class SessionUpdate : Form
    {
        ...
        private void SessionUpdate_Load(object sender, EventArgs e)
        {
            LodingComboLec();
            Select();
        }
        ...
        private void button11_Click(object sender, EventArgs e)
        {
            ...
            {
                SqlCommand cmd3 = new SqlCommand("update NomalSession set sessionID =@sID,lecture = @lec,lectures = @lecs,sub_code = @subcode,subject = @subject,tag = @Tag,grp = @grp,numOfstudent = @nos,duration = @duration where id = @id", con);
                ...
                cmd3.Parameters.AddWithValue("@nos", dataGridView1.Rows[item].Cells[8].Value);
                cmd3.Parameters.AddWithValue("@duration", dataGridView1.Rows[item].Cells[9].Value);
            ...
            {
                con.Open();
                SqlDataAdapter sad = new SqlDataAdapter("SELECT *FROM NomalSession WHERE  sessionID = '" + comboBox1.Text.Trim() + "'", conn);
                DataTable dt = new DataTable();
                _ = sad.Fill(dt);
