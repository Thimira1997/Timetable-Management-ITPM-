...
namespace DakHome
{
    public partial class LocNotAvaUpdate : Form
    {
        public LocNotAvaUpdate()
        {
            InitializeComponent();
        }

        private void LocNotAvaUpdate_Load(object sender, EventArgs e)
        {
            select();
            LodingComboLec();
        }
        ...
        private void button11_Click(object sender, EventArgs e)
        {
            ...
            {
                SqlCommand cmd3 = new SqlCommand("update LocNotAvailable set sessionID =@sID,room = @room,day = @day,stime = @stime,etime = @etime where id = @id", con);
                ...
                cmd3.Parameters.AddWithValue("@day", dataGridView1.Rows[item].Cells[3].Value);
                cmd3.Parameters.AddWithValue("@stime", dataGridView1.Rows[item].Cells[4].Value);
                cmd3.Parameters.AddWithValue("@etime", dataGridView1.Rows[item].Cells[5].Value);
            ...
            {
                con.Open();
                SqlDataAdapter sad = new SqlDataAdapter("SELECT *FROM LocNotAvailable WHERE  sessionID = '" + comboBox1.Text.Trim() + "'", conn);
                DataTable dt = new DataTable();
                _ = sad.Fill(dt);
